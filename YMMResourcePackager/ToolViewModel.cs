    ...
    public class ToolViewModel : BaseViewModel
    {
        ...
        private void UseOpenedProject()
        {
            try
            {
                ...
                {
                    Status = message;
                    MessageBox.Show(message, "諠・ｱ", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }

                SelectedProject = projectPath;
                Status = $"驕ｸ謚・ {SelectedProject}";
                Progress = 0;
            }
            catch (Exception ex)
            {
                Status = $"繧ｨ繝ｩ繝ｼ: {ex.Message}";
                MessageBox.Show(ex.Message, "繧ｨ繝ｩ繝ｼ", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private static bool TryGetOpenedProjectPath(out string projectPath, out string message)
        {
            ...
            {
                message = $"髢九＞縺ｦ縺・ｋ繝励Ο繧ｸ繧ｧ繧ｯ繝亥呵｣懊・隕九▽縺九ｊ縺ｾ縺励◆縺後√ヵ繧｡繧､繝ｫ縺悟ｭ伜惠縺励∪縺帙ｓ:\n{rawPath}";
                return false;
            }
        ...
        private void OpenProjectDialog()
        {
            ...
            {
                Filter = "YMM繝励Ο繧ｸ繧ｧ繧ｯ繝・(*.ymmp)|*.ymmp",
                Title = "プロジェクトを選択"
            };
        ...
        private void AssociateYmmpx()
        {
            try
            {
                ...
                {
                    MessageBox.Show($"繧｢繝励Μ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ:\n{appExe}", "繧ｨ繝ｩ繝ｼ", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
        ...
        private async Task PackageProjectAsync()
        {
            ...
            try
            {
                ...
                var progressReporter = new Progress<YmmpxPackagingProgress>(p =>
                {
                    ...
                    Status = $"ZIP菴懈・荳ｭ... {p.CompletedCount}/{p.TotalCount}";
                });
                ...
                {
                    Progress = 100;
                    Status = $"螳御ｺ・ {outputPath}";

                    MessageBox.Show(
                        $"繝代ャ繧ｱ繝ｼ繧ｸ蛹悶′螳御ｺ・＠縺ｾ縺励◆縲・n\n{outputPath}",
                        "完了",
                        MessageBoxButton.OK,
